...
$(function(){
	...
	$.post("messageCount.action", {uid:uid},function(data){
		if(data==0){
			$(".red").hide();
		}else{
			$(".red").text(data);
...
<div role="navigation" class="zu-top" >
		<div class="zg-wrap modal-shifting clearfix" id="zh-top-inner">
			...
			<c:when test="${not empty sessionScope.loginUser }">
				<a  class="zu-top-nav-userinfo "
					id=":0" role="button" aria-haspopup="true" >
					<span class="name">${sessionScope.loginUser.username }</span> <img class="Avatar"
					src="/pic/${sessionScope.loginUser.photo }"
					
					alt="不死鸟"> <span id="zh-top-nav-new-pm"
				...
				<ul class="top-nav-dropdown" id="top-nav-profile-dropdown">
					...
					<li><a href="${pageContext.request.contextPath }/center.action?id=${sessionScope.loginUser.id}" tabindex="-1" id=":1">
							<i class="zg-icon zg-icon-dd-home"></i>我的主页
					</a></li>

					<li><a href="${pageContext.request.contextPath }/queryMesssage.action?uid=${sessionScope.loginUser.id}" tabindex="-1" id=":2"> <div class="red">${messageCount}</div><i
							class="zg-icon zg-icon-dd-pm"></i>消息 
					</a></li>
					
					<li><a href="${pageContext.request.contextPath }/logout.action" tabindex="-1" id=":4"> <i
				...
				<a  class="zu-top-nav-userinfo "
					...
					<span class="name">登录或注册</span>
				</a>
				<ul class="top-nav-dropdown" id="top-nav-profile-dropdown">
				
				
					<li><a href="${pageContext.request.contextPath }/toRegist.action" tabindex="-1" id=":1">
							<i class="zg-icon zg-icon-dd-home"></i>注册
					</a></li>

					<li><a href="${pageContext.request.contextPath }/toLogin.action" tabindex="-1" id=":2"> <i
							class="zg-icon zg-icon-dd-pm"></i>登录 <span
...
<c:if test="${ cid gt 0}">
			<button class="zu-top-add-question" id="zu-top-add-question" >发帖</button>
</c:if>
			<div id="zg-top-nav" class="zu-top-nav">
				<ul class="zu-top-nav-ul zg-clear">

					<li class="zu-top-nav-li" id="zh-top-nav-home"><a
						class="zu-top-nav-link" href="${pageContext.request.contextPath }/index.action" id="zh-top-link-home"
						>首页</a></li>
				<c:forEach items="${ sessionScope.categoryList}" var="category">
				 <c:choose>
               <c:when test="${cid eq category.id}"> <!-- cid为switchCategory方法传值 -->
               <li class="zu-top-nav-li current" id="zh-top-nav-explore"><a
						class="zu-top-nav-link" href="${pageContext.request.contextPath }/switchCategory.action?cid=${category.id }">${category.name}</a></li>
               </c:when>
               <c:otherwise>
